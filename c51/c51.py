...
def c51(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    def compute_loss_q(data):
        ...
        z_eval = ac.q(o, a)  # (m, N_ATOM)
        # print("fn:{}".format(z_eval.requires_grad))
        if use_gpu:
        ...
        with torch.no_grad():
            z_target = torch.zeros((data_num, N), device=device, dtype=torch.float32)  # (m, N_ATOM)
            # q_pi_targ = ac_targ[idx].q(o2, ac_targ[idx].pi(o2))
            # backup = r + gamma * (1 - d) * q_pi_targ
            # get next state value
            z_next = ac_targ.q(o2, ac_targ.pi(o2))  # (m, N_ATOM)
            ...
            next_v_range = torch.unsqueeze(r, 1) + gamma * torch.unsqueeze((1.0 - d), 1) * torch.unsqueeze(z_range_torch,
                                                                                                          0)
            ...
            """print("lb:\n{}".format(lb))
            print("up:\n{}".format(ub))"""
            tmp1 = z_next * (ub - next_v_pos)
            tmp2 = z_next * (next_v_pos - lb)
            # we didn't vectorize the computation of target assignment.
            for i in range(data_num):
                for j in range(N):
                    # calc prob mass of relative position weighted with distance
                    z_target[i, lb[i, j]] += tmp1[i, j]
                    z_target[i, ub[i, j]] += tmp2[i, j]

        # MSE loss against Bellman backup
        # print("sum of z_target:\n{}\nsum of z_eval:\n{}".format(z_target.sum(dim=-1), z_eval.sum(dim=-1)))
    ...
    def compute_loss_pi(data):
        ...
        if use_gpu:
            o = o.to(torch.device('cuda'))
        # print(ac.q(o, ac.pi(o)))
        q_pi = (ac.q(o, ac.pi(o)) * z_range_torch.unsqueeze(dim=0)).sum(dim=-1)
        # print(q_pi)
        return -q_pi.mean()
    ...
    def get_action(o, noise_scale):
        ...
        if use_gpu:
            o = o.to(torch.device('cuda'))
        a = ac.act(obs=o, use_gpu=use_gpu)
        a += noise_scale * (np.random.randn(act_dim)-0.5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='c51')
    ...
    c51(lambda: gym.make(args.env), actor_critic=core.MLPActorCritic,
